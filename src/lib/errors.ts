...
const SUGGESTIONS: Record<number, string> = {
  401: "Check your token: n8n-cli auth test",
...
export function handleError(err: unknown, json = false): never {
  // Honor --json whether it was passed before or after the subcommand:
  // the preAction hook records the global flag, the action passes its local one.
  json = json || globalFlags.json;
